 ...
 *
 * NOTE: This is a slightly-modified version of the original Thrift version of this
 * class:
 * - Various private methods have been made protected so that they can be used or
 * customized by subclasses.
 * - Slightly optimized version of the writeBinary method used
 *
 */
public class TCompactProtocol extends TProtocol {
  private static final Logger LOGGER = LoggerFactory.getLogger(TCompactProtocol.class.getName());
  static { LOGGER.info("Litelinks-modified "+TCompactProtocol.class.getName()+" class loaded"); }

  // used to identify this modified version via reflection
  public static final boolean LITELINKS_MOD = true;

  protected final static byte[] EMPTY_BYTES = new byte[0];
  protected final static ByteBuffer EMPTY_BUFFER = ByteBuffer.wrap(EMPTY_BYTES);
  ...
  public void writeBinary(ByteBuffer bin) throws TException {
    int length = bin.remaining();
    writeVarint32(length);
    if (bin.hasArray()) {
      trans_.write(bin.array(), bin.position() + bin.arrayOffset(), length);
    } else {
      trans_.write(bin.asReadOnlyBuffer());
    }
  }
   ...
   */
  protected void writeVarint32(int n) throws TException {
    int idx = 0;
    while (true) {
      if ((n & ~0x7F) == 0) {
        temp[idx++] = (byte)n;
   ...
   */
  protected void writeVarint64(long n) throws TException {
    int idx = 0;
    while (true) {
      if ((n & ~0x7FL) == 0) {
        temp[idx++] = (byte)n;
   ...
   * Writes a byte without any possibility of all that field header nonsense.
   */
  protected void writeByteDirect(int n) throws TException {
    writeByteDirect((byte)n);
  }
  ...
  protected void checkStringReadLength(int length) throws TException {
    if (length < 0) {
   ...
   */
  protected int readVarint32() throws TException {
    int result = 0;
    int shift = 0;
   ...
   */
  protected long readVarint64() throws TException {
    int shift = 0;
    long result = 0;
